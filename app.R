...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
          ...
          selectizeInput("tickers", paste("Stocks to Compare:"), choices = NULL,
...
server <- function(input, output, session) {
  ...
  updateSelectizeInput(session, 'tickers', choices = unique(stock_names$Symbol), server = TRUE)
    ...
    output$stockTable <- renderDataTable({
      ...
      InitialValues = prices %>% group_by(prices$symbol) %>% filter(row_number() == 1) %>% rename(initialValue = adjusted)
      LastValues = prices %>% filter(date == max(date)) %>% rename(finalValue = adjusted)
      ...
      initial_last_values <- initial_last_values[, c("symbol",
                                                    'Name',
                                                    'initialValue',
                                                    'finalValue',
                                                    'changeByPercentage')]
